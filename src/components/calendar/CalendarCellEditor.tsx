        ...
        {isWeekend ? (
          ...
          <>
            <FieldInput
              ...
              inputRef={firstInputRef}
            />
            <FieldInput label="ER↑" value={getValue("er_am", duty?.er_am)} onChange={(v) => onFieldChange("er_am", v)} placeholder="담당자" />
            <FieldInput label="ER↓" value={getValue("er_pm", duty?.er_pm)} onChange={(v) => onFieldChange("er_pm", v)} placeholder="담당자" />
            <FieldInput label="당직" value={getValue("night_duty", duty?.night_duty)} onChange={(v) => onFieldChange("night_duty", v)} placeholder="담당자" />
          </>
        )}
        <FieldInput label="저널" value={getValue("journal_presenter", journal?.presenter)} onChange={(v) => onFieldChange("journal_presenter", v)} placeholder="발표자" />
        <FieldInput
          label="NGR"
          value={getValue("ngr_info", ngr && (ngr.schedule_info || ngr.person) ? `${ngr.schedule_info} - ${ngr.person}` : "")}
